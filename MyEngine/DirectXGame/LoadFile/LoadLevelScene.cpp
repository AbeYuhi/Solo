	...
	for (auto& object : deserialized["objects"]) {
		...
		if (type.compare("MESH") == 0) {
			...
			if (object.contains("children")) {
				ScanChildData(levelData.get(), object, static_cast<int32_t>(levelData->objects.size()) - 1);
			}
		}
...
void ScanChildData(LevelData* levelData, json& childrens, int32_t parent) {
	for (auto& object : childrens["children"]) {
		...
		if (type.compare("MESH") == 0) {
			...
			if (object.contains("children")) {
				ScanChildData(levelData, object["children"], static_cast<int32_t>(levelData->objects.size()) - 1);
			}
		}
	}
}std::unique_ptr<LevelData> LoadFile(std::string directoryPath, std::string fileName) {
